# ============================================================
# TrackOrder - Script de Deploiement GCP (GKE)
# Projet: gestion-de-livraison-493711
# Usage:
#   .\deploy-gcp.ps1                         -> deploiement complet
#   .\deploy-gcp.ps1 -SkipBuild -SkipPush -SkipClusterCreate  -> deploy seul
#   .\deploy-gcp.ps1 -DestroyCluster         -> supprimer le cluster
# ============================================================
...
# gcloud ecrit ses messages sur stderr (comportement normal de gcloud)
# On garde Continue pour ne pas crasher sur ces messages
...
Write-Host "  TrackOrder - Deploiement GCP (GKE)"       -ForegroundColor Cyan
...
# ---------- Suppression du cluster (si demande) ----------
if ($DestroyCluster) {
    Write-Host "[DESTROY] Suppression du cluster $ClusterName..." -ForegroundColor Red
    gcloud container clusters delete $ClusterName --zone $Zone --quiet 2>&1 | Write-Host
    Write-Host "[DONE] Cluster supprime." -ForegroundColor Green
    exit 0
}
...
# Rediriger stderr vers stdout pour eviter que PowerShell le traite comme erreur
...
if (-not $SkipBuild) {
    ...
    Write-Host "[3/8] Build skip (-SkipBuild)." -ForegroundColor DarkGray
}
...
if (-not $SkipPush) {
    ...
    Write-Host "[4/8] Push skip (-SkipPush)." -ForegroundColor DarkGray
}
...
if (-not $SkipClusterCreate) {
    ...
    # Lister les clusters et chercher le nom exact (suppression des warnings stderr)
    ...
    Write-Host "[5/8] Creation cluster skip (-SkipClusterCreate)." -ForegroundColor DarkGray
}
...
Write-Host ""
Write-Host "  Attente de l'IP externe du LoadBalancer (max 60s)..." -ForegroundColor Gray
$maxWait = 60
$waited  = 0
$externalIp = ""
do {
